...
class studentclass:
  def __init__(self,root):
    ...
    ##title
    title=Label(self.root,text=" Student Details",font=("JetBrains Mono",18,"bold"),bg="#033054",fg="white").place(x=10,y=15,width=1180,height=35)
  ######variables#####
    self.var_roll=StringVar(self.root) ##### self root kerna parega add baqio pe bhi bhai
    ...
    self.var_dob=StringVar(self.root)
    self.var_contact=StringVar(self.root)
    self.var_course=StringVar(self.root)
    self.var_a_date=StringVar(self.root)
    self.var_state=StringVar(self.root)
    self.var_city=StringVar(self.root)
    self.var_pin=StringVar(self.root)
    ...
    lbl_roll=Label(self.root,text="RollNo",font=(" goudy old style",15,"bold"),bg='white').place(x=10,y=60)
    ...
    txt_state=Entry(self.root,textvariable=self.var_state,font=(" goudy old style",15),bg='lightyellow').place(x=150,y=220,width=150)

    lbl_city=Label(self.root,text="City",font=(" goudy old style",15,"bold"),bg='white').place(x=310,y=220)
    txt_city=Entry(self.root,textvariable=self.var_state,font=(" goudy old style",15),bg='lightyellow').place(x=380,y=220,width=100)

    lbl_pin=Label(self.root,text="Pin",font=(" goudy old style",15,"bold"),bg='white').place(x=500,y=220)
    txt_pin=Entry(self.root,textvariable=self.var_state,font=(" goudy old style",15),bg='lightyellow').place(x=570,y=220,width=120)
...
#########entryfields####
    ...
    self.txt_gender=ttk.Combobox(self.root,textvariable=self.var_gender,values=("Select","Male","Female"),font=(" goudy old style",15),state='readonly',justify=CENTER)
...
####entry fields c2
    ...
    ##calling
    self.fetch_course()
    txt_dob=Entry(self.root,textvariable=self.var_dob,font=(" goudy old style",15),bg='lightyellow').place(x=480,y=60,width=200)
    txt_contact=Entry(self.root,textvariable=self.var_contact,font=(" goudy old style",15),bg='lightyellow').place(x=480,y=100,width=200)
    txt_admission=Entry(self.root,textvariable=self.var_a_date,font=(" goudy old style",15),bg='lightyellow').place(x=480,y=140,width=200)
    ...
    lbl_search_roll=Label(self.root,text="Roll No",font=(" goudy old style",15),bg='white').place(x=720,y=60)
    txt_search_roll=Entry(self.root,textvariable=self.var_search,font=(" goudy old style",15),bg='lightyellow').place(x=870,y=60,width=180)
...
######continue from video #3
            ...
            if row==None:
              messagebox.showerror("Error","please select the student from the list",parent=self.root)
               ...
               if op==TRUE:
                  ...
                  con.commit()
                  messagebox.showinfo("Succes the student is deleted",parent=self.root)
                  self.clear()
  ...
  def get_data(self,ayan):
    self.txt_roll.config(state="readonly")   ###index zero se hay
    ...
    try:
      if self.var_roll.get()=="":
        ...
        if row!=None:
          ...
          con .commit()
          messagebox.showinfo("student updated succesfully",parent=self.root)
          self.show()
